		...
		// Redirect to specific page if not logged in	
			...
			if ( ! is_user_logged_in() ) {
				...
				if ( is_array( ghostpool_option( 'login_register_page_redirect_exclusion' ) ) ) {
					...
					if ( is_page( $excluded_pages ) OR ( function_exists( 'bp_is_active' ) && ( bp_is_register_page() OR bp_is_activation_page() ) ) ) {
						return;
					}
				}
				...
				if ( $excluded_urls = ghostpool_option( 'login_register_page_redirect_url_exclusion' ) ) {
					...
					foreach( $excluded_urls as $excluded_url ) {
						...
						$clean_excluded_urls[] = '/' . rtrim( $excluded_url, '/' ) . '/';
					}
					
					global $wp;
					$page_url = add_query_arg( null, null );
					
					if ( in_array( $page_url, $clean_excluded_urls, true ) ) {
						return;
					}					
	...
	function ghostpool_login() {
		...
		if ( isset( $_POST['action'] ) && $_POST['action'] == 'ghostpool_login' ) {
			...
			if ( is_wp_error( $user_verify ) ) {
				...
				// Let third party plugins/code hook to it
				$login_redirect = apply_filters( 'ghostpool_login_redirect', '', $user_data );

				// If the redirect was not specified via filter, let us check if we have a page to redirect to
				if ( ! $login_redirect && $login_redirect_page_id ) {
					$login_redirect = get_permalink( $login_redirect_page_id );
				}

				// If we have the login redirect url specified, lets use it
				if ( ! empty( $login_redirect ) ) {
					$js_output = 'window.location.replace("' . $login_redirect . '");';
				} else {
					// Otherwise, refresh the page
					$js_output = 'window.location.reload();';
				}
				
				echo '<script data-cfasync="false" type="text/javascript">' . $js_output. '</script>';
